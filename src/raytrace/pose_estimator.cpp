    ...
    double xy_opt[2], double eval_opt[1])
{
  ...
  {
    for(double y(y0-ry); y<y0+ry; y+= 2.0*ry/40.0)
    {
      obj->Move(x,y,z0);
      double sqdiff_depth(0.0), sqdiff_normal(0.0), eval(0.0);
      GetDistance(depth_img, normal_img, sqdiff_depth, sqdiff_normal, step_xp, step_yp);
    ...
    double z_opt[1], double eval_opt[1])
{
  ...
  for(double z(z0-rz); z<z0+rz; z+= 2.0*rz/40.0)
  {
    obj->Move(x0,y0,z);
    double sqdiff_depth(0.0), sqdiff_normal(0.0), eval(0.0);
    GetDistance(depth_img, normal_img, sqdiff_depth, sqdiff_normal, step_xp, step_yp);
