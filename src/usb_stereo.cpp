...
{
  int n_disp= (((img_size_.width/8) + 15) & -16)+16*4;
  ...
  sp_.StereoMethod          = TStereoSGBMParams::smSGBM;
  sp_.GrayScale             = true;
}
//-------------------------------------------------------------------------------------------
...
void TStereo::Init()
{
  ...
  cv::stereoRectify(cp_.K1, cp_.D1, cp_.K2, cp_.D2, img_size_, cp_.R, cp_.T.t(),
      cp_.R1, cp_.R2, cp_.P1, cp_.P2, cp_.Q, /*flags=*/cv::CALIB_ZERO_DISPARITY, /*alpha=*/0.0
      /*,Size newImageSize=Size(), Rect* validPixROI1=0, Rect* validPixROI2=0*/);
  std::cerr<<"cp_.Q="<<cp_.Q<<std::endl;
  cv::initUndistortRectifyMap(cp_.K1, cp_.D1, cp_.R1, cp_.P1, img_size_, CV_16SC2, map11_, map12_);
  cv::initUndistortRectifyMap(cp_.K2, cp_.D2, cp_.R2, cp_.P2, img_size_, CV_16SC2, map21_, map22_);

  if(sp_.StereoMethod==TStereoSGBMParams::smBM)
  {
    stereo_bm_.init(sp_.preset, sp_.numberOfDisparities, sp_.SADWindowSize);
  }
  else if(sp_.StereoMethod==TStereoSGBMParams::smSGBM)
  {
