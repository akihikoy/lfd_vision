...
def Run(t,args=()):
  ...
  lg_x_ext= t.control_frame[t.whicharm] #Local vector to the current control frame
  ...
  while rospy.Time.now() < start_time + rospy.Duration(duration):
    ...
    x_curr= t.CartPos(lg_x_ext)
    x_trg= AverageX(x_curr, x_ar, smoothing_factor)
    t.MoveToCartPosI(x_trg,time_step,lg_x_ext,inum=30,blocking=True)
